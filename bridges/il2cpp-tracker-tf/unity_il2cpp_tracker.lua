...
-- Poll until IL2CPP runtime is initialized; calling API functions before
...
local function resolve_icall(name)
    ...
    -- Validate icall name against compiled-in allowlist, then whitelist
    -- the pre-resolved address for native.call.
...
local function resolve_all_icalls()
    ...
    -- _Injected variants take an output-param pointer; the struct is
    -- written into caller-supplied memory rather than returned.
...
-- Pre-allocated output buffers for _Injected methods (written by Unity,
-- sized to 16 bytes for alignment).
local vec3_buf = nil
local quat_buf = nil
...
local function try_read_scene_name()
    ...
    -- _Injected variant writes the Scene struct to scene_buf.
...
local function handle_tick()
    ...
    -- Heartbeat keeps the host aware; no fatal-error on null camera since
    -- the engine detects process exit and we just retry next tick.
...
local function parse_message(message)
    -- Lua backend delivers recv messages as raw JSON strings.
    ...
    -- JS backend delivers an already-parsed table.
