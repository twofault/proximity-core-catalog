-- Unity IL2CPP camera tracker. Pull-tick: host sends ticks, agent replies.
...
-- Net-id-capture helper. Loaded after main agent; non-fatal on failure.
...
local function attach_with_retries()
    ...
    -- Wait for lingering Frida worker from prior detach to drain.
    -- Yields keep the engine thread responsive.
...
function init()
    ...
    -- Retry attach to absorb early-process startup races.
    ...
    local load_result_handle, load_result_err = Gamelink.loadScript("unity_il2cpp_tracker.lua", {
        runtime = "lua",
    })
    ...
    -- Optional net-id-capture helper.
...
function update(dt)
    ...
    -- Single-drain poll avoids the multi-handle queue race.
    ...
    if messages then
        for _, msg in ipairs(messages) do
            ...
            if msg.type == "data" and msg.payload then
                ...
                -- Pass forward+up basis directly so audio skips euler
                -- decomposition (degenerate at zenith/nadir, caused world-spin
                -- on look-up). Unity is +Z forward, ours is -Z, so negate Z.
                if d.upX ~= nil then
                ...
                -- Unity scene name -> GameStore level. Only write on change
                -- to keep per-tick log noise down.
