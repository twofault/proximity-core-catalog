-- Terraria agent: pure memory scan + read. No CLR hosting, no native.call.
...
-- Terraria pixel coords: X ~1000-134000, Y ~500-38000.
-- Both near zero = uninitialized player, reject.
...
local function validate_candidate(hit)
    ...
    -- Elements start after the length field (+ 64-bit padding).
    local base = hit + (IS_64 and 8 or 4)

    -- Sample 20 elements: same MT across most = real Player[].
    local ref_mt = nil
    ...
    for i = 0, math.min(19, 255) do
        ...
        if elem then
            ...
            if off then
                -- Deep check: many more elements must share the MT.
                local deep_same = 0
                for j = 20, 255, 10 do
...
local function handle_tick(msg)
    ...
    -- Heartbeat = alive, no position. Engine handles process exit.
