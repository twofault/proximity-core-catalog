// C# reflection bridge for the Frida CLR REPL. Compiled at runtime by
// repl.py with .NET 4.0 csc.exe — no C# 6+ syntax allowed (no ?., no pattern
// matching, no string interpolation). Loaded via ExecuteInDefaultAppDomain;
// entry must be static int Execute(string). Results written to
// %TEMP%\frida_clr_bridge\bridge_result.json.
...
namespace FridaBridge
{
    public class Bridge
    {
        ...
        // BOM breaks Frida's File.readAllText.
        static readonly Encoding Utf8NoBom = new UTF8Encoding(false);

        // Signature required by ExecuteInDefaultAppDomain.
        ...
        static string CmdFields(string typeName)
        {
            ...
            foreach (FieldInfo f in fields)
            {
                ...
                {
                    int offset = -1;
                    // Marshal.OffsetOf on a managed class can fault — value types only.
        ...
        static string CmdReadAddr(string path)
        {
            ...
            // Resolve all but the final segment, then introspect the last field.
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(fieldName))
                {
                    ...
                    // Instance field: pin parent, then compute field address.
                ...
                // Fallback: pin the resolved value directly.
                object resolved;
                Type resolvedType;
        ...
        // Path resolution e.g. "Terraria.Main.player[0].position.X".
        static void ResolvePath(string path, out object value, out Type valueType)
        {
            ...
            // Greedy: longest type-name prefix that resolves wins.
            Type type = null;
            int fieldStart = 0;
            ...
            // Walk remaining segments as field/property accesses.
            object current = null;  // null = static context
