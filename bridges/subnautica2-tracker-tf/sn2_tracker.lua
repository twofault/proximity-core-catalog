...
-- Restore the ReflectionWalker module's state from a cached payload's
-- `reflection` sub-table (populated by `_buildDiscoveryState`). Without
-- this the SN2 probe can't enumerate pawn properties on cached attaches
-- because `getAllProperties` dereferences `ReflectionWalker.offsets`.
--
-- Accepts either top-level `data.reflection` (newer schema) or nested
-- `data.discoveryState.reflection` (older schema) so users with stale
-- caches don't have to wait through a fresh discovery.
local function rehydrate_reflection_state(data)
    if type(data) ~= "table" then return false end
    local r = type(data.reflection) == "table" and data.reflection
        or (type(data.discoveryState) == "table"
            and type(data.discoveryState.reflection) == "table"
            and data.discoveryState.reflection)
        or nil
    if not r then return false end
    ReflectionWalker.offsets = {
        childProps  = r.childProps  or -1,
        children    = r.children    or -1,
        superStruct = r.superStruct or -1,
        structSize  = r.structSize  or -1,
    }
    ReflectionWalker.fieldOffsets = {
        ["next"]   = r.fieldNext  or -1,
        name       = r.fieldName  or -1,
        fieldClass = r.fieldClass or -1,
    }
    ReflectionWalker.propOffsets = {
        offsetInternal = r.propOffsetInternal or -1,
    }
    -- Mirror the convenience fields the rest of the agent reads off the module.
    ReflectionWalker._uFieldNextOffset = r.fieldNext or -1
    ReflectionWalker._fieldClassOffset = r.fieldClass or -1
    ReflectionWalker.useFProperty = r.useFProperty
    ReflectionWalker.vectorPrecision = r.vectorPrecision
    return true
end
...
local function handle_init(data)
    ...
    if type(data) == "table" and type(data.offsets) == "table" then
        ...
        local refl_restored = rehydrate_reflection_state(data)
        if refl_restored then
            log("UE Engine (Lua): ReflectionWalker offsets restored from cache "
                .. "(SN2 probe will use them)")
        else
            log("UE Engine (Lua): cache has no `reflection` block — SN2 probe will "
                .. "skip property reads until next full discovery")
        end
...
-- ── Subnautica 2 game-specific probe ──
--
-- In UE, piloting a vehicle SWAPS the PlayerController's possessed Pawn —
-- the on-foot character pawn becomes a vehicle pawn. So "what's the local
-- player doing" is mostly answered by the pawn's CLASS, plus a handful of
-- properties on each class. We resolve property offsets lazily per class
-- and cache by class address (UObjects don't move, so cache-by-address is
-- stable for the engine's lifetime).
--
-- Designed to survive game updates: NAMES are reflection metadata that
-- tracks the source code; OFFSETS shift every build but the agent re-
-- discovers them from names on each new class encountered.
local Sn2Probe = {
    -- Per-class offset cache.
    --   class_cache[class_addr] = {
    --       class_name = string,
    --       is_vehicle = bool,
    --       off_cmc      = int (-1 if absent) — CharacterMovement ref
    --       off_piloting = int (-1 if absent) — vehicle ref on pawn
    --   }
    -- For CharacterMovementComponent classes we cache a different shape:
    --   class_cache[cmc_class_addr] = {
    --       is_cmc_class = true,
    --       class_name = string,
    --       off_movement_mode = int,
    --   }
    class_cache = {},
    -- Stable cache of class names by object address — used to avoid
    -- calling UObjectLayout:getObjectName on every vehicle-ptr read.
    name_cache = {},
}

-- Per-property pattern table. Each entry is { types, names } — only
-- properties whose UE type is in `types` AND whose lowercase name
-- substring-matches one of `names` count.
--
-- HISTORY: earlier versions also resolved `RecentlySubmerged`,
-- `InBaseGracePeriod`, `CurrentOxygenator`, `bInBaseReplication`. All
-- four turned out to be unreliable on SN2 early access:
--
--   * `RecentlySubmerged` is a debounced tag that lingers after surfacing
--     and inside habitats — false positives for underwater.
--   * `InBaseGracePeriod` reads the Class Default Object value (0.15)
--     even outside any base — false positive for in-base.
--   * `CurrentOxygenator` and `bInBaseReplication` stayed false in all
--     tested scenes — no signal.
--
-- Today we use only:
--   * `CharacterMovement.MovementMode` (UE built-in, MOVE_Swimming = 4)
--     for underwater detection — see CMC_PROPERTY_NAMES below.
--   * Pawn class name + `piloting` property below for vehicle detection.
--
-- "In-base" classification is deliberately not implemented — no reliable
-- signal on `BP_Character_01_C` was found. Audio still reacts to vehicles
-- (Tadpole etc.) and underwater. Base-versus-open-air is the same
-- surroundings profile until a stable in-base signal turns up.
local SN2_FIELD_PATTERNS = {
    -- Vehicle reference on the player character. SN2 doesn't re-possess
    -- the pawn for vehicles like the Tadpole — the character stays as the
    -- pawn and references the vehicle via a property. ObjectProperty types
    -- cover hard / weak / soft refs.
    piloting = {
        types = { ["ObjectProperty"] = true, ["WeakObjectProperty"] = true,
                  ["SoftObjectProperty"] = true },
        names = { "pilotingvehicle", "pilotedvehicle", "currentvehicle",
                  "mountedvehicle", "drivenvehicle", "pilotedpawn",
                  "currentlyriding", "currentlypiloting", "interactingvehicle",
                  "vehicle" },
    },
}

-- Property names whose presence alone suggests "this class is a vehicle".
-- Used to flag the pawn as a vehicle when its name doesn't match a known
-- pattern (Hoverbike, Seatruck, Drone, etc.).
local VEHICLE_MARKER_PROPERTIES = {
    "driverdata", "drivercollision", "pilotingthrottle",
    "vehiclemovementcomponent", "drivelimitarea",
}

-- UE Character.MovementMode enum values. Reliable cross-game underwater
-- signal — Subnautica 2's player Character switches to MOVE_Swimming when
-- the head goes below the water surface.
local MOVE_NONE      = 0
local MOVE_WALKING   = 1
local MOVE_NAVWALK   = 2
local MOVE_FALLING   = 3
local MOVE_SWIMMING  = 4
local MOVE_FLYING    = 5
local MOVE_CUSTOM    = 6

-- Names we accept for the CharacterMovementComponent reference on a
-- Character-subclass pawn, in priority order.
local CMC_PROPERTY_NAMES = { "charactermovement", "movementcomponent" }
-- Names we accept for the MovementMode enum on the CMC.
local MOVEMENT_MODE_NAMES = { "movementmode" }

local function lower_match_any(prop_name_lc, patterns)
    for _, p in ipairs(patterns) do
        if prop_name_lc:find(p, 1, true) then return p end
    end
    return nil
end

-- Resolve property offsets for a single class. First name+type match wins
-- per slot. Walks the entire super-struct chain so inherited properties
-- (UE built-ins on Character / Pawn) are visible. Result is cached per
-- class address — UObjects don't move, so the cache lives until detach.
function Sn2Probe.probeClass(self, cls)
    local cached = self.class_cache[cls]
    if cached then return cached end

    local class_name = UObjectLayout:getObjectName(cls) or "<unknown>"

    -- Guard: reflection-by-property requires ReflectionWalker.offsets, which
    -- isn't always present (cached attach without the `reflection` block).
    -- Degrade to a name-only classification rather than crashing the tick.
    if not ReflectionWalker.offsets then
        local lcname = class_name:lower()
        local is_vehicle_named =
            lcname:find("seatruck",   1, true)
            or lcname:find("tadpole",   1, true)
            or lcname:find("hoverbike", 1, true)
            or lcname:find("vehicle",   1, true)
            or lcname:find("prawn",     1, true)
            or lcname:find("droneturret", 1, true)
            or lcname:find("submarine", 1, true)
        local info = {
            class_name = class_name,
            property_count = 0,
            is_vehicle = is_vehicle_named and true or false,
            off_cmc      = -1,
            off_piloting = -1,
            reflection_unavailable = true,
        }
        log(string.format(
            "SN2: class '%s' name-only classification (reflection unavailable, vehicle=%s)",
            class_name, tostring(info.is_vehicle)))
        self.class_cache[cls] = info
        return info
    end

    local props = ReflectionWalker:getAllProperties(cls)

    local function resolve(field_key)
        local spec = SN2_FIELD_PATTERNS[field_key]
        for _, p in ipairs(props) do
            if p.name and (not spec.types or not p.type or spec.types[p.type]) then
                local lc = p.name:lower()
                local match = lower_match_any(lc, spec.names)
                if match then return p.offset, p.name end
            end
        end
        return -1, nil
    end

    -- "Is this class a vehicle" detection — either it has a marker property
    -- (driver/pilot/vehicle component), or its name matches a known pattern.
    local is_vehicle_marker = false
    for _, p in ipairs(props) do
        if p.name then
            local lc = p.name:lower()
            for _, marker in ipairs(VEHICLE_MARKER_PROPERTIES) do
                if lc:find(marker, 1, true) then is_vehicle_marker = true; break end
            end
            if is_vehicle_marker then break end
        end
    end
    local lcname = class_name:lower()
    local is_vehicle_named =
        ...
        or lcname:find("droneturret", 1, true)
        or lcname:find("submarine", 1, true)
    local is_vehicle = is_vehicle_marker or (is_vehicle_named and true or false)

    local off_piloting, name_piloting = resolve("piloting")

    -- Diagnostic dump of properties matching vehicle/piloting/interaction
    -- patterns. Lets us spot any vehicle-reference property we haven't
    -- mapped yet on a class we haven't seen before. One-shot per class.
    log("SN2:   diagnostic dump of vehicle-related props (first 20):")
    local dumped = 0
    for _, p in ipairs(props) do
        if p.name and dumped < 20 then
            local lc = p.name:lower()
            if lc:find("vehicle", 1, true)
                or lc:find("pilot", 1, true)
                or lc:find("drive", 1, true)
                or lc:find("ride", 1, true)
                or lc:find("mount", 1, true)
                or lc:find("seat", 1, true)
                or lc:find("dock", 1, true)
                or lc:find("tadpole", 1, true)
            then
                log(string.format("SN2:     +0x%04X %-22s %s",
                    p.offset or 0, p.type or "?", p.name))
                dumped = dumped + 1
            end
        end
    end

    -- CharacterMovementComponent reference, by name. Type is ObjectProperty
    -- (or its Weak/Soft variants in some builds). Substring match so renames
    -- like `MyMovementComponent` still resolve — the standard UE name is
    -- `CharacterMovement` which matches "charactermovement".
    local off_cmc, name_cmc = -1, nil
    for _, p in ipairs(props) do
        if p.name and (p.type == "ObjectProperty" or p.type == "WeakObjectProperty"
            or p.type == "SoftObjectProperty") then
            local lc = p.name:lower()
            for _, cname in ipairs(CMC_PROPERTY_NAMES) do
                if lc:find(cname, 1, true) then
                    off_cmc, name_cmc = p.offset, p.name
                    break
                end
            end
            if off_cmc >= 0 then break end
        end
    end

    -- A non-vehicle character pawn without a CMC reference means swimming
    -- detection will silently fail. Log once at probe time so the user can
    -- see the regression rather than wonder why water never muffles.
    if off_cmc < 0 and not is_vehicle then
        log(string.format(
            "SN2: WARN class '%s' has no CharacterMovement-like ObjectProperty — "
            .. "swimming/underwater detection will be inactive for this pawn",
            class_name))
    end
    ...
    local info = {
        ...
        off_cmc      = off_cmc,      name_cmc      = name_cmc,
        off_piloting = off_piloting, name_piloting = name_piloting,
    }
    log(string.format(
        "SN2: class '%s' (%d props, vehicle=%s) cmc=%s piloting=%s",
        class_name, #props, tostring(is_vehicle),
        tostring(name_cmc), tostring(name_piloting)))
    self.class_cache[cls] = info
    return info
end

-- Resolve MovementMode offset for a CharacterMovementComponent class.
-- Cached per CMC class — most pawns share one CMC class.
function Sn2Probe.probeCmcClass(self, cmc_cls)
    if not cmc_cls or cmc_cls == 0 then return nil end
    local cached = self.class_cache[cmc_cls]
    if cached and cached.is_cmc_class then return cached end
    if not ReflectionWalker.offsets then return nil end

    local class_name = UObjectLayout:getObjectName(cmc_cls) or "<unknown>"
    local props = ReflectionWalker:getAllProperties(cmc_cls)
    local off_mm, name_mm = -1, nil
    for _, p in ipairs(props) do
        if p.name and (p.type == "ByteProperty" or p.type == "EnumProperty"
            or p.type == "Int8Property") then
            local lc = p.name:lower()
            for _, mname in ipairs(MOVEMENT_MODE_NAMES) do
                if lc == mname then
                    off_mm, name_mm = p.offset, p.name
                    break
                end
            end
            if off_mm >= 0 then break end
        end
    end
    local info = {
        is_cmc_class = true,
        class_name = class_name,
        off_movement_mode = off_mm,
        name_movement_mode = name_mm,
    }
    log(string.format("SN2: CMC class '%s' (%d props) movement_mode=%s@0x%X",
        class_name, #props, tostring(name_mm), off_mm >= 0 and off_mm or 0))
    self.class_cache[cmc_cls] = info
    return info
end

function Sn2Probe.classNameForAddr(self, addr)
    if not addr or addr == 0 then return nil end
    local cached = self.name_cache[addr]
    if cached then return cached end
    local cls = UObjectLayout:readClass(addr)
    if not cls then return nil end
    local name = UObjectLayout:getObjectName(cls) or "<unknown>"
    self.name_cache[addr] = name
    return name
end

-- Read game-state fields off the currently-possessed pawn. Returns a table;
-- missing fields are absent so the host can detect via `field == nil`.
function Sn2Probe.read(self, pawn)
    local out = {}
    if not pawn or pawn == 0 then return out end

    local cls = UObjectLayout:readClass(pawn)
    if not cls then return out end
    local info = self:probeClass(cls)

    out.pawnClassName = info.class_name
    out.pawnIsVehicle = info.is_vehicle

    -- CharacterMovementComponent.MovementMode — the reliable underwater
    -- signal for Character-derived pawns. MOVE_Swimming = 4. The CMC is
    -- a separate UObject reachable via the pawn's CharacterMovement ref.
    if info.off_cmc >= 0 then
        local cmc = MemUtil.readPtr(pawn + info.off_cmc)
        if cmc and cmc ~= 0 then
            local cmc_cls = UObjectLayout:readClass(cmc)
            if cmc_cls then
                local cmc_info = self:probeCmcClass(cmc_cls)
                if cmc_info and cmc_info.off_movement_mode >= 0 then
                    local mm = MemUtil.readU8(cmc + cmc_info.off_movement_mode)
                    if mm ~= nil then
                        out.movementMode = mm
                        out.isSwimming = (mm == MOVE_SWIMMING)
                    end
                end
            end
        end
    ...
    -- Piloting reference. SN2 doesn't re-possess for the Tadpole — the
    -- character stays as the pawn, and a vehicle reference lives in a
    -- character property (PilotingVehicle / CurrentVehicle / etc.). When
    -- non-null, override the vehicle flag so the host treats us as piloting.
    if info.off_piloting >= 0 then
        local v = MemUtil.readPtr(pawn + info.off_piloting)
        if v and v ~= 0 then
            out.pilotingVehicle = v
            out.pilotingVehicleClassName = self:classNameForAddr(v)
            out.pawnIsVehicle = true  -- override: we're effectively in a vehicle
        end
    end
...
local function handle_tick(message)
    ...
    elseif type(data_or_err) == "table" then
        -- Augment with SN2 game state. Pawn class changes when the player
        -- enters/exits vehicles (UE re-possession), so Sn2Probe caches per
        -- class — read() probes-on-demand the first time a new class is
        -- encountered.
        local pawn = PositionReader.cachedAddresses and PositionReader.cachedAddresses.Pawn
        if pawn and pawn ~= 0 then
            local ok_read, sn2 = pcall(Sn2Probe.read, Sn2Probe, pawn)
            if ok_read and type(sn2) == "table" then
                for k, v in pairs(sn2) do data_or_err[k] = v end
            elseif not ok_read then
                log("SN2: probe read error: " .. tostring(sn2))
            end
        end
        data_or_err.protocol = "subnautica2_tracker"
        send(data_or_err)
        state.consecutiveErrors = 0
