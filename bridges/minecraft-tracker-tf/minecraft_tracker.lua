-- Minecraft Java Edition tracker. Reads player pos/rotation via JNI.
-- Read-only: field getters only (no Call*Method), no NewGlobalRef
-- (player re-read each tick). JNI vtable whitelisted via
-- native.lookup("jvm.dll", "jni::Name", addr); JIT accessor stubs live
-- in executable heap (not jvm.dll).

local PTR = 8 -- 64-bit JVM only

-- JNINativeInterface_ vtable slots.
local JNI = {
...
-- JNIInvokeInterface_ vtable slots.
...
local function verify_fn(name, index)
    ...
    -- jni:: prefix triggers any-module check; JIT stubs live in heap.
...
-- GetFloatField uses XMM0; fall back to GetIntField + bit reinterpret.
...
local function init_jvm()
    ...
    -- Whitelist AttachCurrentThread before invoking.
    ...
    -- Required before any JNI call from this thread.
...
local function do_tick()
    ...
    -- Heartbeat only; engine detects process exit, so keep retrying silently.
