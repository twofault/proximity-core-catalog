-- Minecraft Java Edition bridge. Auto-deobfuscates via Mojang mappings.
...
-- Gimbal-lock-safe orientation forwarding. At pitch +/-90 the basis is
-- degenerate against yaw, so float noise spins audio "up" at zenith/nadir.
-- Clamp pitch just under +/-90 (HRTF diff is sub-perceptual) and hold the
-- last stable yaw when inside the unstable cone AND the new yaw jumps past
...
local function await_http(handle, timeout_ms, progress_msg, progress_target, half_life)
    ...
    -- handle:result() returns (data, err); normalize to 3-tuple matching
    -- other return paths so callers can use `if not success`.
    local data, err = handle:result()
    if data then
        return true, data, nil
    end
    return false, nil, err or "HTTP request failed"
end

-- Some modpacks ship a window title with no version (e.g. "RLCraft" vs
-- "Minecraft 1.12.2"). The mappings download needs a Mojang version ID,
-- so map known modpacks to their underlying version. Best-effort: wrong
-- guesses still fail later with a clearer error.
local MODPACK_VERSION_MAP = {
    ["RLCraft"] = "1.12.2",
}

local function extract_version(text)
    if not text then return nil end
    -- Modpack name match takes priority -- modpack titles like "RLCraft"
    -- have no digits but ARE pinned to a known version.
    for modpack_name, mc_version in pairs(MODPACK_VERSION_MAP) do
        if text:find(modpack_name, 1, true) then
            return mc_version
        end
    end
...
-- Client classes unobfuscated from 1.26.1 onward.
...
local function parse_mappings(mappings_text, progress_start, progress_end)
    ...
    for i, line in ipairs(lines) do
        ...
        -- Class line: "net.minecraft.client.Minecraft -> abc:"
...
function init()
    ...
    if window_title then
        ...
        if detected_version then
            ...
            -- Window title present but no version extractable. Clear error
            -- beats a later Json.decode(nil) crash. Common: modpack titles
            -- without version (ATM10) or launchers that override format
            -- (Curseforge, MultiMC, Prism, technic).
            Core.error("Could not detect Minecraft version from window title \""
                .. window_title .. "\". This bridge needs the Minecraft version " ..
                "to download mappings. If you're playing a modpack, the title " ..
                "may not include the underlying Minecraft version -- please " ..
                "set the version manually in the modpack's launcher options, " ..
                "or open an issue with the title \"" .. window_title .. "\" so " ..
                "we can add a special case.")
            Bridge.shutdown("Minecraft version not detected from window title")
            return
        end
    else
        Core.error("Could not read window title for PID " .. tostring(pid)
            .. ". The Minecraft window may not be ready yet -- try again " ..
            "after the game finishes loading to the main menu.")
        Bridge.shutdown("Minecraft window title unavailable")
        return
    end
    ...
    do
        ...
        -- If we detected a version but found no mappings, assume unobfuscated.
        if detected_version then
...
function update(dt)
    ...
    -- Lua backend has no setInterval; main.lua drives updates at 60Hz.
    ...
    if messages then
        for _, msg in ipairs(messages) do
            if msg.type == "data" and msg.payload then
                ...
                -- Log agent fatal but don't disconnect; engine detects exit,
                -- Gamelink.isError() above catches Frida errors.
                ...
                if d.yaw then
                    -- MC pitch: + = down, - = up. Ours: + = up. Negate.
                    local pitch_rad = -math.rad(d.pitch or 0)

                    -- MC yaw: 0 = south (+Z), CW. Ours: 0 = -Z, CCW.
                    -- Mapping: our_yaw = pi - mc_yaw.
...
function dispose()
    ...
    -- Runtime cleans up Gamelink automatically.
    script_handle = nil
end
