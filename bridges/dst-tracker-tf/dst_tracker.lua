-- dst_tracker.lua — Don't Starve Together Camera Tracker (Compliant)
--
-- Flat read-only security model: memory.read_* + native.observe only.
-- NO interceptor, NO memory writes, NO code execution in game Lua VM.
--
-- Architecture:
--   1. Find lua_pcall address via export scan + luaL_Reg pattern scan
--   2. native.observe(lua_pcall) to discover active lua_State* pointers
--   3. Walk Lua 5.1 internal memory structures:
--      lua_State.l_gt → globals hash → "ThePlayer" → entity table
--        → "entity" field (lightuserdata) → C++ Entity → Transform → position
--      lua_State.l_gt → globals hash → "TheCamera" → camera table
...
local TVALUE_SIZE = 16  -- sizeof(TValue) on both 32/64-bit MSVC Lua 5.1

-- lua_State struct offsets (Lua 5.1, MSVC, verified layout)
local OFF = {}
if IS_X64 then
    ...
    OFF.UDATA_DATA = 40  -- offset of user payload in full userdata
else
...
local function validate_table(tbl)
    ...
    if not lsn or lsn > 30 then return false end  -- 2^30 nodes = absurd
...
local function find_lua_pcall()
    ...
    -- Strategy 2: luaL_Reg table scan
    ...
    -- luaB_pcall calls: luaL_checkany, lua_gettop, ... , lua_pcall (typically 3rd+ call)
...
local function probe_position(base, scan_size)
    ...
    -- One level of indirection: follow pointers in the object
...
local state = {
    ...
    lua_pcall_addr = nil,
    observer = nil,
    captured_states = {},       -- { [addr_str] = { ptr, count } }
    main_state = nil,           -- lua_State* with ThePlayer
    globals_tbl = nil,          -- Table* for globals
    player_tbl = nil,           -- Table* for ThePlayer entity
    camera_tbl = nil,           -- Table* for TheCamera
    entity_ptr = nil,           -- C++ Entity* from ThePlayer.entity
    pos_probe = nil,            -- probe result for position reading
    ...
    ticks = 0,
    init_start = 0,
    init_timeout = 45000,       -- 45 seconds to discover everything
    last_diag = 0,
}
...
local function do_probe_states()
    ...
    -- Need at least one state with some activity before probing
...
-- Forward declarations (defined below do_discover)
...
local function do_discover()
    ...
    -- Try each captured state, starting with the most active
    local sorted = {}
        ...
        if globals and validate_table(globals) then
            ...
            if ptt == LUA_TTABLE and pval then
                ...
                elseif ctt == LUA_TUSERDATA and cval then
                    ...
                    -- Camera might be a class instance with userdata backing
                    state.camera_tbl = nil  -- can't walk userdata hash
                else
