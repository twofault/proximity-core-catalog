...
-- Extracts camera position from Unity Mono games via Frida Lua runtime.
...
function init()
    ...
    local loaded = Frida.getLoadedScripts()
    if loaded and #loaded > 0 then
        ...
        local load_result = Frida.load("unity_mono_tracker.lua", {
            runtime = "lua",
            capability = "invoke",
        })
