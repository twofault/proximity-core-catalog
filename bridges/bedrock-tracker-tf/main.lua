-- Minecraft Bedrock bridge: camera (Vec3 + forward unit) lives in exe's
-- static data. Offset auto-discovered, cached per exe hash.
...
-- Gimbal guard: near pitch ±90° yaw becomes degenerate against the forward/up
-- basis and atan2(0,0) noise spins the audio "up" vector. Clamp pitch just
-- under 90° (sub-perceptual HRTF cost) and hold last stable yaw when inside
-- the unstable cone if yaw jumps past the rejection threshold.
            ...
            for _, msg in ipairs(messages) do
                ...
                if msg.type == "data" and msg.payload then
                    ...
                    if d.type == "progress" then
                        -- Agent reports 0-100; map to host's 70-99% window.
        ...
        for _, msg in ipairs(messages) do
            ...
            if msg.type == "data" and msg.payload then
                ...
                if d.posX then
                    ...
                    -- Bedrock = Java coords: X east, Y up, Z south. Pass thru.
                    GameStore.setCameraPosition(d.posX, d.posY, d.posZ)

                    -- Negate fwdX/fwdZ: flip 180° on XZ plane.
